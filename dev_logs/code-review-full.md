...
### C2. 出貨的 docker-compose 直接帶 Turnstile「永遠通過」測試金鑰
...
### C3. Docker socket 掛進後端容器 = 後端被打穿就等於拿到宿主機 root
...
### C4. 錯誤處理路徑本身會拋 `UnboundLocalError`
...
### H4. `/build` 的去重機制有 race condition
...
#### M1. Center Console 設定抓取沒有 timeout
...
#### M2. `reload_settings()` 會讓所有 JWT 失效
...
#### M6. Root logger 吸收所有第三方 log 並轉送到 PostHog
...
#### M7. `shutil.rmtree` 在 async 函式裡阻塞
...
#### M8. `Alert.tsx` 違反 Rules of Hooks + 計時器沒清理
...
#### M12. `runBtn.tsx` 在 render 期間讀 DOM 並改動 ref
...
#### M14. docker-compose 的環境變數名稱和程式碼對不上
...
#### M16. 前端 Dockerfile 在容器啟動時才 build
...
#### M17. 完全沒有測試，CI 也不做 lint / typecheck
...
| # | 檔案 | 說明 |
|---|---|---|
| L1 | `backend/router/verify.py:30` | `datetime.utcnow()` 自 Python 3.12 起已 deprecated（Dockerfile 用的是 3.14），且回傳 naive datetime。改用 `datetime.now(timezone.utc)` |
| L2 | `backend/utils/cache.py:17` | `hash_id: str = Field(default=None, ...)` — 型別標注是 `str` 但預設 `None`。目前都有顯式賦值所以沒事，但型別是假的 |
| L3 | `backend/router/verify.py:45` | `need_token` 的型別標注是 `dict`（`Depends(need_token)` 處），實際回傳 `bool` |
| L4 | `backend/router/build.py:136` | `_in_flight` 是無界 dict。正常路徑會 pop，但 [H4](#h4-build-的去重機制有-race-condition) 的 race 會讓項目遺留 |
| L5 | `frontend/src/layouts/Layout.astro:29` | `lang="en"` 寫死在所有頁面上，包含 zh-TW 版。螢幕閱讀器會用錯語音，SEO 也會誤判。應該用 `Astro.currentLocale` |
| L6 | `frontend/src/pages/404.astro:11-14` | 用 `<meta http-equiv="refresh">` + `location.replace()` 把所有 404 導回首頁。破壞深層連結的錯誤訊息，且 meta refresh 是無障礙反模式 |
| L7 | `frontend/src/layouts/Layout.astro:100-117` | Google Analytics 與 PostHog 都無條件載入，沒有 consent gate。若有歐盟流量會有 GDPR 問題 |
| L8 | `frontend/src/components/share.tsx:56` | `history.replaceState(null, "", "/editor")` 寫死路徑，會把使用者的語言前綴（`/zh-tw/editor`）洗掉 |
